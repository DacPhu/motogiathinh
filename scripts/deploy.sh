...
# VPS_HOST, VPS_USER, VPS_PASS, VPS_PORT, REMOTE_DIR
set -euo pipefail
...
SSH_OPTS="-p ${VPS_PORT} -o StrictHostKeyChecking=no -o ConnectTimeout=10 \
  -o ControlMaster=auto -o ControlPath=${SSH_CONTROL} -o ControlPersist=120"

if [[ -n "${VPS_PASS:-}" ]]; then
  command -v sshpass >/dev/null || die "sshpass not found — brew install hudochenkov/sshpass/sshpass"
  export SSHPASS="${VPS_PASS}"
  SSH="sshpass -e ssh ${SSH_OPTS}"
  RSYNC="sshpass -e rsync -e 'ssh ${SSH_OPTS}'"
else
  SSH="ssh ${SSH_OPTS}"
  RSYNC="rsync -e 'ssh ${SSH_OPTS}'"
fi

trap 'ssh -p ${VPS_PORT} -o ControlPath=${SSH_CONTROL} -O exit ${SSH_TARGET} 2>/dev/null || true; rm -f "${SSH_CONTROL}"' EXIT

remote() { eval "$SSH" "${SSH_TARGET}" "$@"; }
sync_file() { eval "$RSYNC" "$@"; }
...
# Frontend: pre-built dist + slim Dockerfile (skips npm on server)
...
# Nginx, docker-compose, and production overrides
cp "${LOCAL_ROOT}/nginx/nginx.conf"      "${BUILD_DIR}/nginx/nginx.conf"
cp "${LOCAL_ROOT}/docker-compose.yml"    "${BUILD_DIR}/docker-compose.yml"
cp "${LOCAL_ROOT}/docker-compose.prod.yml" "${BUILD_DIR}/docker-compose.prod.yml" 2>/dev/null || true
...
sync_file -az "${ZIP_PATH}" "${SSH_TARGET}:${REMOTE_ZIP_DIR}/${ZIP_NAME}"
...
# ── Copy .env (preserve existing on server) ───────────────────────────────────
step "Checking .env on server"
remote bash -s <<REMOTE
if [ ! -f "${REMOTE_DIR}/.env" ]; then
  if [ -f "${REMOTE_DIR}/.env.example" ]; then
    cp "${REMOTE_DIR}/.env.example" "${REMOTE_DIR}/.env"
    echo "  ⚠ Created .env from .env.example — fill in production values!"
  else
    echo "  ⚠ No .env found — create ${REMOTE_DIR}/.env before services will start"
  fi
else
